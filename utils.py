...
def generate_batches_lol(batch_size, file_path):
    ...
    for start_idx in range(0, len(dataset), batch_size):
        ...
        padded_batch_x = [
            sublist + [0] * (max_length - len(sublist)) for sublist in batch_sublists
        ]
...
class SequenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.add_one_token:
            ...
            return torch.tensor(x, dtype=torch.int64), torch.tensor(
                y, dtype=torch.int64
            )
...
class SequenceDataset_lol(Dataset):
    ...
    def __init__(self, filepath, add_one_token=True):
        self.data = [
            seq for seq in pickle.load(open(filepath, "rb"))
        ]  # Loading the sequences as lists
        self.add_one_token = add_one_token
